class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        for num, count in freq.items(): # grouping num and count together grouped on the basis of their respective counts
            buckets[count].append(num)  # buckets represent frequencies, thus frequencies are ordered from low to high
            #freq = Counter([1, 1, 1, 2, 2, 3])
            # freq = {1: 3, 2: 2, 3: 1}
        
        res = []
         # Traverse buckets from high to low frequency because buckets become ordered in ascending so we get the largest frequencies at the end. 
